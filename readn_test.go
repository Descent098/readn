...
func TestFleschKincaid(t *testing.T) {
	...
	if math.Ceil(float64(res.Ease)) != 57 {
		t.Fatalf("FleschKincaid gave an incorrect answer %.2f rounded up does not equal 57", res.Ease)
	}
	if math.Ceil(float64(res.Level)) != 14 {
		t.Fatalf("FleschKincaid gave an incorrect answer %.2f rounded up does not equal 14", res.Level)
	}

	res2 := readn.FleschKincaid(paradiseLostBookOne)

	if math.Ceil(float64(res2.Ease)) != 40 {
		t.Fatalf("FleschKincaid gave an incorrect answer %.2f rounded up does not equal 40", res2.Ease)
	}
	if math.Ceil(float64(res2.Level)) != 22 {
		t.Fatalf("FleschKincaid gave an incorrect answer %.2f rounded up does not equal 22", res2.Level)
	}
}
...
func TestSMOG(t *testing.T) {
	...
	if math.Ceil(float64(res2)) != 14 {
		t.Fatalf("SimpleMeasureOfGobbledygook() gave an incorrect answer %.2f rounded up does not equal 14", res2)
	}
...
func TestARI(t *testing.T) {
	...
	if res2 != 27 {
		t.Fatalf("AutomatedReadabilityIndex() got an incorrect value for text %d should be 27", res2)
	}
}
...
func TestInfoParsing(t *testing.T) {
	...
	if res2.Sentences != 112 {
		t.Fatalf("GetTextData() gave an incorrect number of sentences %.f does not equal 112", res2.Sentences)
	}
